        ...
        <div class="card invoice1">
            <div class="body">
                <div class="invoice-top clearfix">
                    ...
                    <div class="info">
                        ...
                        <p><strong>Mobile:</strong> {{ $payments->msisdn }} <br>
                            ...
                            {{ number_format($payments->trans_amount, 2,'.',',') }}<br>
                    ...
                    <div class="title">
                        ...
                        <strong>Date:</strong> {{ $payments->trans_date }}
                ...
                <div class="invoice-top clearfix">
                    ...
                    <div class="title">
                        ...
                        <strong>Payment Confirmed:</strong> <span
                            class="badge badge-success">{{ $trans_confirmed }}</span><br>
                        @else
                        <strong>Payment Confirmed:</strong> <span
                            class="badge badge-warning">{{ $trans_confirmed }}</span><br>
                        @endif
                        @if($cron_processed == 'Processed')
                        <strong>Payment Processed:</strong> <span
                            class="badge badge-success">{{ $cron_processed }}</span><br>
                        @else
                        <strong>Payment Processed:</strong> <span
                            class="badge badge-warning">{{ $cron_processed }}</span><br>
                        @endif
                    </div>
                </div>
                <hr>
                ...
                <div class="row clearfix">
                    <div class="col-md-6">
                        <h5>Payment Note</h5>
                        <p>Payment for rent by {{ $payments->first_name}} {{ $payments->last_name }} through mobile
                            number {{ $payments->t_phone}} at
                            {{ $payments->trans_date}}. <br> Mpesa confirmation code {{ $payments->trans_id }} </p>
                    </div>
                    <div class="col-md-6 text-right">
                        <p class="m-b-0"><b>Sub-total: </b>{{ $currency_symbol }}
                            {{number_format($payments->trans_amount, 2, '.', ',')}}</p>
                        <p class="m-b-0">Discount: 0.00%</p>
                        <p class="m-b-0">VAT: 0.00%</p>
                        <h3 class="m-b-0 m-t-10">{{ $currency_symbol }}
                            {{number_format($payments->trans_amount, 2, '.', ',')}}</h3>
                    </div>
